// NEW renderFatigueWidget function - verified standalone
// This replaces the old one in home.js
...
renderFatigueWidget() {
    ...
    return '\n      <div class="fatigue-widget" style="background:var(--color-bg-card);border:1px solid var(--color-border);border-radius:16px;padding:16px;margin-bottom:16px">\n'
      ...
      + '        </div>\n'
      + (function() {
        const high = Object.entries(fatigue).filter(([,v]) => v > 50).sort((a,b) => b[1]-a[1]);
        if (!high.length) return '';
        return '<div style="display:flex;flex-wrap:wrap;gap:4px;justify-content:center;margin-top:6px">'
          + high.map(([name, pct]) => '<span style="font-size:0.65rem;padding:3px 10px;border-radius:10px;background:' + getFatigueColor(pct) + ';color:var(--color-text);cursor:pointer;font-weight:500" onclick="HomeScreen.showMuscleDetail(\'' + name + '\')">' + name + ' ' + pct + '%</span>').join('')
          + '</div>';
      })()
